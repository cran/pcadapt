...
Sections A-E explain how to use the package for genotype data. Section F additionally explains how to use the package for Pool-seq data, but previous sections should also be read. Section G explains advanced usage of the package.
...
```
./PCAdapt fast -i outputgenotype -o myoutput -K number_of_PCs
```

where `outputgenotype` is the genotype matrix, `myoutput` is the name of the output file and `number_of_PCs` is the number of principal components that will be computed. Choose a value of `K` that is large enough (`K>20`) in order to be able to choose `K` with the scree plot.
...
When the optimal value of `K` is found, you can compute Mahalanobis distances and p-values using again the `pcadapt` function.
