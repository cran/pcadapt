...
- `zscores` is a `(L,K)` matrix containing the normalized linear regression coefficients.
...
**NB**: it is important to add the extension ".pcadapt" to the output file name.
...
**NB**: please make sure the files "myoutput", "outputgenotype", "myoutput.scores", "myoutput.loadings" and "myoutput.sigma" are all contained in the same folder.
